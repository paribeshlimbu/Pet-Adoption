...
app.use(cors())
webpush.setVapidDetails('mailto:', process.env.VAPID_KEY_PUBLIC, process.env.VAPID_KEY_PRIVATE);
...
/**
 * Notify users when a pet is updated
...
function notifyUsersUpdate (message) {
    ...
    if (message.transaction === 'delete') {
        mailBody = 'The pet has been deleted and therefore deleted from your favorites.';
        notificationBody = 'The pet has been deleted and therefore deleted from your favorites.';
    } else if (message.sold) {
        mailBody = 'The pet has been sold. Click in the button below to see details in Petadoption.';
        notificationBody = 'The pet has been sold. Click in the button below to remove it from favorites.';
        notificationActions.push({ action: 'delete', title: 'Delete' });
    } else if (message.booked) {
        mailBody = 'The pet is now booked. Click in the button below to see details in Petadoption.';
        notificationBody = 'The pet is now booked. Click in the button below to see details and contact the owner.';
        notificationActions.push({ action: 'detail', title: 'Detail' });
    } else if (parseFloat(message.price) < parseFloat(message.oldPrice)) {
        mailBody = `The pet has dropped in price (from ${message.oldPrice} to ${message.price}). Click in the button below to see details in Petadoption.`;
        notificationBody = `The pet has dropped in price (from ${message.oldPrice} to ${message.price}). Click in the button below to see details and contact the owner.`;
        notificationActions.push({ action: 'detail', title: 'Detail' });       
    } else if (parseFloat(message.price) > parseFloat(message.oldPrice)) {
        mailBody = `The pet has risen in price (from ${message.oldPrice} to ${message.price}). Click in the button below to see details in Petadoption.`;
        notificationBody = `The pet has risen in price (from ${message.oldPrice} to ${message.price}). Click in the button below to navigate to your favorites.`;
        notificationActions.push({ action: 'detail', title: 'Detail' });       
    }
...
function notifyUsersCreate (message) {
    ...
    // Build the message
    const mailBody = `New pet that may be of your interest created recently. ${message.name} with a selling price of ${message.price}. Click the button to see it in Petadoption.`;
    ...
    .then(res => {
        res.forEach(advert => {
            ...
            if (subscription) {
                try {
                    pushNotification(subscription, message, 'New pet for you', notificationBody, notificationActions);                       
                } catch (error) {
                    mailNotification(advert.user.name, advert.user.email, 'New pet for you', message, mailBody);   
                }
            }
            else {
                mailNotification(advert.user.name, advert.user.email, 'New pet for you', message, mailBody);
